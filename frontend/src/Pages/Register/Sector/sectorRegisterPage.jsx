...
import { listSector, updateSector, deleteSector, createSector } from '../../../contexts/SectorProvider/actions';
import { Container } from '../styles';

function SectorRegisterPage({formInputs}) {
    ...
    const loadData = useCallback(() => {
        ...
        listSector(sectorContext.dispatch).then((dispatch) =>{
    ...
    },[sectorContext.dispatch, navigate]);
    ...
    const handleClick = () => {
        ...
        if(selected){
            updateSector(sectorContext.dispatch, formData, selected['id']);
        }else{
            createSector(sectorContext.dispatch, formData);
        };
    ...
    //     deleteSector(sectorContext.dispatch, selected['id'], selected['name']);
    //     reloadScreen();
    // };
    ...
    return (
        <MainFormContainer
            ...
            loading={sectorContext.state.loading}
        
        >
            {
                // After an event on this page , shows the feedback message
                sectorContext.state.feedback && (
                    <Container>
                        <SnackBarComponent 
                            title={sectorContext.state.feedback}
                            type={sectorContext.state.error ? 'FAIL' : 'SUCCESS'}
                        />  
                    </Container>
                ) 
            }
...
export default SectorRegisterPage;
