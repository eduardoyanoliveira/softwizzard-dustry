...
import { listBreakReason, createBreakReason, updateBreakReason, deleteBreakReason } from '../../../contexts/BreakReasonProvider/actions';
import { Container } from '../styles';

function BreakReasonRegisterPage({formInputs}) {
    ...
    const loadData = useCallback(() => {
        ...
        listBreakReason(breakReasonContext.dispatch).then((dispatch) =>{
    ...
    },[breakReasonContext.dispatch, navigate]);
   ...
   // Capture "AUTOCOMPLET" chosen object and update the values of all inputs ,
    ...
    const handleClick = () => {
        ...
        if(selected){
            updateBreakReason(breakReasonContext.dispatch, formData, selected['id']);
        }else{
            createBreakReason(breakReasonContext.dispatch, formData);
        };
    ...
    //     deleteBreakReason(breakReasonContext.dispatch, selected['id'], selected['name']);
    //     reloadScreen();
    // };

    return (
        <MainFormContainer
            title={'Cadastro de motivo de Parada'}
            loading={breakReasonContext.state.loading}
            isRestrict={false}
        >
            {
                // After an event on this page , shows the feedback message
                breakReasonContext.state.feedback && (
                    <Container>
                        <SnackBarComponent 
                            title={breakReasonContext.state.feedback}
                            type={breakReasonContext.state.error ? 'FAIL' : 'SUCCESS'}
                        />  
                    </Container>
                ) 
            }
...
export default BreakReasonRegisterPage;
